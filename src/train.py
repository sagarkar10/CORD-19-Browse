'''
This train file is to show a training piplein, this is not used to generate the
model for covid19. This infact uses cs papers from arxiv in xml files.
'''
import pandas as pd
import xmltodict
import fasttext
...
def read_xmls(filenames):
    ...
    mdf["dc:subject"] = mdf["dc:subject"].apply(
        lambda x: [x] if isinstance(x, str) else x
    )
    mdf["dc:creator"] = mdf["dc:creator"].apply(
        lambda x: [x] if isinstance(x, str) else x
    )
    mdf["dc:date"] = mdf["dc:date"].apply(
        lambda x: [x] if isinstance(x, str) else x
    )
    mdf["dc:identifier"] = mdf["dc:identifier"].apply(
        lambda x: [x] if isinstance(x, str) else x
    )
    mdf["dc:description"] = mdf["dc:description"].apply(
        lambda x: x[0] if isinstance(x, list) else x
    )
    mdf["dc:title"] = mdf["dc:title"].apply(
        lambda x: x if isinstance(x, str) else x
    )
    mdf = mdf.rename(
        columns={
