...
def _get_token_similarity(query_string, pred_string):
    ...
    for k, v in query_vec.items():
        dist = cdist(
            [v], np.stack(list(pred_vec.values()), axis=0), metric="cosine"
        )[0]
        idx = dist.argsort()[:2]
...
def get_most_similar_title(query_title, df, top_n=5):
    ...
    dist = cdist(
        [v0], np.stack(df.title_vect.values, axis=0), metric="cosine"
    )[0]
    ...
    ret = dict(
        {"query": query_title, "processed_query": " ".join(tokens), "pred": {}}
    )
